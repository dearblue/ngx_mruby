...
mrb_value ngx_mrb_start_fiber(ngx_http_request_t *r, mrb_state *mrb, struct RProc *rproc, mrb_value *result)
{
  ...
  *fiber_proc = mrb_funcall_with_block(mrb, mrb_obj_value(mrb_class_get(mrb, "Fiber")), mrb_intern_lit(mrb, "new"), 0, NULL, mrb_obj_value(rproc));
  if (mrb->exc) {
...
mrb_value ngx_mrb_run_fiber(mrb_state *mrb, mrb_value *fiber_proc, mrb_value *result)
{
  ...
  resume_result = mrb_fiber_resume(mrb, *fiber_proc, 0, NULL);
  if (mrb->exc) {
